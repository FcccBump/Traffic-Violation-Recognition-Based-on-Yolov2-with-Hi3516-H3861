...
0701：
    2200：
        向 cloud 发送前增加了一个发送 起始标志的东西 buf_begin 。
    2300：
        设计了两个变量标志 socket 是否连接 cloud 和 app 。
        因为没有连接成功 cloud ，返回值 ret 就是小于0的，刚好初始化函数的返回值就是 ret ，所以会导致整个程序崩溃。所以在
        连接 cloud 后设置了 ret=0 ，防止干扰。
        所以现在即使不连接 app 和 cloud ，也可以跑，音频功能也还在。
0709：
    0100：
        增加了 3516 和 3861 通过 wifi 通信的代码，这样就可以由 3516 直接控制 3861 警示灯高亮。但是例程编译会出现警告。
    1930：
        跑十几分钟就会崩溃。不知道为啥。可能是增加了 3516 和 3861 通过 wifi 通信的代码。
    2025：
        注释了把摄像头获取的 yuv 图片保存到板端的一个函数。
    2100：
        运行没有加 3516 和 3861 通过 wifi 通信的代码，也会崩溃。发现修改了部署的参数后，使用的模型不匹配，导致的错误。画
        框那里就是错的，这个会导致程序崩溃。
    2300：
        替换为正确的模型后就对了，但是发现跑了10分钟后还是会出现无法打开 app.jpg 文件的 error ， tcp socket 创建
        也 error 。这两个错误不会导致程序崩溃。
0710：
    1330：
        经过优化，跑了20分钟，不联网下没有报错。
    1920：
        使用 socket 接收 3861 发送的红绿灯状态。
0711：
    0156：
        现在实现 3516 和 3861 全用 socket 通信。所以把之前 wifi 看不懂的发送接收例程全部删除了。
0713：
    2106：
        多类别模型部署完成，修改了 ai_infer_process.c 中的东西，就是两个函数。
        分析模型输出的初步结果的函数 Yolo2FetchRes() ，
        以及模型软件参数初始化函数 SampleSvpNnieYolov2SoftwareInit()。
        修改为模型能识别三个类别即可。
        这里注意！！！因为现在用的模型是能识别三个类型的，所以这里的软件参数设置中必须也设置为3，否则这种不匹配会导致板子
        跑一段时间就崩溃。
    2312：
        将图片格式转换、保存到磁盘的步骤优化一下，只有需要发送才会做这两个操作。
        发现模型运行效率很高，11帧发一帧， cloud 收到的图片大概的平均间隔是1.5s，所以一帧小于0.15s。这时引入
        给 app 传输图片，cloud 收到的图片大概的平均间隔是4.7s，所以给 app 传输11帧图片的整个流程需要3.2s。根据之前的经验，
        给 app 传输图片几乎不用时间，主要延时在于格式转换以及保存到磁盘。
0718:
    2130:
        1、模型部署的置信度设置了0.6，测试基本能过，发现穿黑色短裤的行人的大腿会被识别为车轮！！！可能需要进一步增大置信度。

        2、设置了每30s中的后8s内如果没有违规情况，那么最多播放两遍宣传语音，如果有违规情况可能会导致只有一次或者没有宣传语音。
        为了在公司宿舍调试，在 
Z:\home\openharmony\device\soc\hisilicon\hi3516dv300\sdk_linux\sample\platform\common\sample_comm_audio.c 
        中的函数 SAMPLE_INNER_CODEC_CfgAudio() 中把音频分贝改小为-5dB。

        3、我用板子测试跑了两小时，没有开启网络相关功能，只有模型识别和音频，没有问题。
*/
...
// #define SERVER_IP1 "10.138.249.88"// my 热点 0711_1939
#define SERVER_IP1 "10.140.198.73"// my 热点 0713_2251
...
// #define SERVER_IP "10.138.249.88"// my 热点 0711_1939
#define SERVER_IP "10.140.198.73"// my 热点 0713_2251
// udp 相关如上
...
static pthread_t g_cooperate_audioProcessThread = 0;
static OsdSet* g_osdsTrash = NULL;
static HI_S32 g_osd0Trash = -1;
static HI_S32 global_audio_flag = 0;
static HI_S32 propaganda_slogans = 0;
static HI_S32 play_propaganda_slogans_0 = 0;// 控制30s中的后8s内若无违规情况则播放宣传语音
static HI_S32 play_propaganda_slogans_1 = 0;// 保证30s中的后8s内才选择宣传语音类别。
static HI_S32 play_propaganda_slogans_2 = 0;// 控制30s中的后8s内播放次数，现在设定为2次。
// 音频相关如上
...
static int sockfd1_available;// APP
static int sockfd2_available;// Huawei_cloud
...
static int global_helmet_violation_0 = 0;// 用作音频播放的标志
static int global_helmet_violation_1 = 0;// 用作获取音频类别的标志
static int global_helmet_violation_2 = 0;// 用作控制给 cloud 发送违规图片的标志
static int global_trafficlight_violation_0 = 0;// 用作音频播放的标志
static int global_trafficlight_violation_1 = 0;// 用作获取音频类别的标志
static int global_trafficlight_violation_2 = 0;// 用作给 3861 发送信息来高亮警示灯的标志
static float cnnScores[DETECT_OBJ_MAX];
// ai_control_audio 相关如上

// ai_control_cloud 相关如下
static int count_cycle = 0;
static int count_cycle_1 = 0;
...
// 3516 和 3861 wifi_tcp_socket 通信如下
#define SERVER_PORT3 8888 // 3861 连接 3516 的端口号，用于接收 3861 发送过来的数据
#define SERVER_PORT4 9999 // 3861 连接 3516 的端口号，用于向 3861 发送数据
static pthread_t wifi_tcp_socket_ProcessThread_1 = 0;
static pthread_t wifi_tcp_socket_ProcessThread_2 = 0;
static int red_state = 0;
// 3516 和 3861 wifi_tcp_socket 通信如上
 ...
 */
static HI_VOID PlayAudio2(const RecogNumInfo items)// 受手势识别模型的输出参数 global_helmet_violation_0 的控制
{
    // if  (g_count < AUDIO_FRAME) {// AUDIO_FRAME = 14 。相当于进入 PlayAudio2() 函数的前14次都直接返回，第15次进
    // // 入才满足条件可以播放音频。还是进入该函数后先计数到14才执行下面的程序？显然是前者！！！因为 return 是直接结束函
    // // 数了。只有执行完成 PlayAudio2 函数，最后才有一个 g_count = 0 ，表示下次播放需要重新计数了。
    //     g_count++;
    //     return;
    // }
    const RecogNumInfo *item = &items;
    uint32_t score = item->score * MULTIPLE_OF_EXPANSION / SCORE_MAX;// 1800*100/4096
    if ( (score > AUDIO_SCORE) && (global_helmet_violation_0 != 0 || global_trafficlight_violation_0 != 0) 
        && (global_audio_flag == 0) ) {
        // 满足条件才会执行音频播放程序。 AUDIO_SCORE = 40 ，定义在前面 。所以 item->score 最小为1639。
        // global_helmet_violation_0 = 0;global_trafficlight_violation_0 = 0;global_audio_flag = 1;
        g_num = item->num;
        printf("播放开始时： global_helmet_violation_0 = %d, global_trafficlight_violation_0 = %d, global_audio_flag = %d \n", 
            global_helmet_violation_0, global_trafficlight_violation_0, global_audio_flag);
        if (g_num != UNKOWN_WASTE) {// UNKOWN_WASTE = 20
            AudioTest(g_num, -1);// 在 audio_test.c 中，482。 g_num 是识别出来的垃圾类别。现在是需要播放的音频类别，一
            // 共两种。分别是 1：“请佩戴头盔”，2：“请不要闯红灯”。
            // 当 AudioTest() 以及其下层函数播放对应音频文件后才有 return ，结束函数回到这里。 
        }
        global_helmet_violation_0 = 0;global_trafficlight_violation_0 = 0;global_audio_flag = 1;
        global_audio_flag = 0;
    }
    else if(play_propaganda_slogans_0 == 1 && global_audio_flag == 0 && play_propaganda_slogans_2 < 2){
        play_propaganda_slogans_2 ++;
        global_audio_flag = 1;
        g_num = item->num;
        if (g_num <= 7 && g_num >= 3) {
            AudioTest(g_num, -1);// 播放文明宣传标语
        }
        global_audio_flag = 0;
        printf("播放文明宣传标语结束 SUCCESS \n");
    }
    g_count = 0;
}
...
*/
    ...
    // 但是还有下层函数，所以不是一直播放
        ...
        if(global_helmet_violation_1 != 0){
            resBuf.num = 1;// 播放“请佩戴头盔”。优先级更高
            // //
            // resBuf.num = 2;// 为了测试播放“请不要闯红灯”。
            // //
            global_helmet_violation_1 = 0;
        }
        else if(global_trafficlight_violation_1 != 0){
            resBuf.num = 2;// 播放“请不要闯红灯”。
            global_trafficlight_violation_1 = 0;
        }
        else if(play_propaganda_slogans_1 == 1){// play_propaganda_slogans_1 保证每30s的后8s才会进入。
            if(propaganda_slogans == 3){
                resBuf.num = 3;// 播放“文明每一步，安全每一天”。
            }
            else if(propaganda_slogans == 4){
                resBuf.num = 4;// 播放“出行多点小心，家人少点担心”。
            }
            else if(propaganda_slogans == 5){
                resBuf.num = 5;// 播放“和谐社会和谐路，文明交通文明人”。
            }
            else if(propaganda_slogans == 6){
                resBuf.num = 6;// 播放“遵守交通法规，关爱生命旅程”。
            }
            else if(propaganda_slogans == 7){
                resBuf.num = 7;// 播放“红绿灯，斑马线，交通标志要看好”。
            }
        }
        PlayAudio2(resBuf);// 受手势识别模型的输出参数 objs 的间接控制。
        // 如果满足播放条件，那么是会等播放结束，再一层一层返回直到返回到这里的。
        
        // 这个线程没有延时，所以可能有问题，现在表现出来的就是音频播放只能播放第一类（由第一个 if 控制），所以手动延
        // 时一下。
        // usleep(1000);// 配合全局变量 g_count 计数到14
        usleep(10000);// 不用全局变量 g_count 计数
    }
    return NULL;
}

static HI_VOID* GetCooperateAudioFileName(HI_VOID* arg)
{
    int ret = 0;int i = 0;
    propaganda_slogans = 3;
    play_propaganda_slogans_0 = 0;
    play_propaganda_slogans_1 = 0;
    while(1){
        if(propaganda_slogans > 7){
            propaganda_slogans = 3;
        }
        sleep(22);
        play_propaganda_slogans_0 = 1;play_propaganda_slogans_1 = 1;
        sleep(8);
        play_propaganda_slogans_0 = 0;play_propaganda_slogans_1 = 0;play_propaganda_slogans_2 = 0;
        propaganda_slogans ++;
    }
}
/* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ 音频相关如上 ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */ 
...
{// 该函数没问题了，所以把一些 printf 注释了
    ...
    if ((outfile = fopen(filename, "wb")) == HI_NULL) {
        printf("yuv_to_111_jpg error can't open %s\n", filename);
        return -1;
    }
...
{// 该函数没问题了，所以把一些 printf 注释了
    // printf("send_to_111_Huawei_cloud1 读取硬盘 jpg 图片再 tcp 发送给 cloud 流程开始！\n");
    int ret;
    ...
    */
    ...
    // if ((tocloud_fp = fopen(jpgFileNamepointer, "r")) == HI_NULL) {// right，r表示只读
        printf("send_to_111_Huawei_cloud1 can't open, error \n");
        return -1;
    }
    else{// 打开文件成功
        ...
        上述这三个 char 的定义会报错，具体哪一个还不清楚！！！感觉是第一个就错了。确实是。
        */
        // HI_CHAR buf_end[IVE_FILE_NAME_LEN];HI_CHAR buf_begin[IVE_FILE_NAME_LEN];
        ...
        */
        ...
        if(buf_max_length>length) {
            ...
            // ret = send(sockfd2, buf_begin, strlen(buf_begin), 0);// 发送开始标志，这样可以
            // if(0 > ret){printf("send buf_begin error");}
            // printf("send begin_flag over\n");
            ...
            ret = send(sockfd2, buf_end, strlen(buf_end), 0);// 发送结束标志
            if(0 > ret){printf("send buf_end error   send buf_end error   send buf_end error \n");}
            // printf("strlen(buf_end)=%d   send end_flag over \n", strlen(buf_end));
            for(int i=0; i<1; i++){
                printf("给云发送图片以及标志位的流程结束 SUCCESS \n");
            }
        }
    ...
    // printf("send_to_111_Huawei_cloud1 读取硬盘 jpg 图片再 tcp 发送给 cloud 流程结束！\n");
    return 0;
}
...
{// 该函数没问题了，所以把一些 printf 注释了
    // printf("sendto_111_app1 以下读取硬盘 jpg 图片再 udp 发送给 app 流程开始 \n");
    ...
    if(send_to_APP==1){
        ...
        if (fp == NULL){
            for(int i = 0; i < 3; i ++){
                printf("sendto_111_app1 File:\t%s Not Found! error 传输无法开始 \n", sendto_APP_file_name0);
            }
            return -1;
        }
        else{
            ...
            for(int i = 0; i < 1; i ++){
                printf(" ............................. sendto_111_app1 udp_sendto_APP start ............................. \n");
            }
    ...
    // printf("sendto_111_app1 以上读取硬盘 jpg 图片再 udp 发送给 app 流程结束 \n");
    return 0;
}
/* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ sendto_app above ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */ 
...
// ///////////////////////////////////////// 3516 和 3861 wifi_tcp_socket 通信如下
static void *wifi_tcp_socket_3516_3861_1(void *arg)// 接收 3861 发送的数据
{// 该函数没问题了，所以把一些 printf 注释了
    int ret;
    // for(int i = 0; i < 2; i++){
    //     printf("wifi_tcp_111_socket_3516_3861 循环，为了接收 3861 传来的红绿灯状态 \n");
    // }

    ////////////////////////////////////////// 等待 3861 连接 3516 如下 //////////////////////////////////////////
    struct sockaddr_in server_addr = {0};
    struct sockaddr_in client_addr = {0};
    int sockfd_light_state, connfd;
    int addrlen = sizeof(client_addr);

    /* 打开套接字，得到套接字描述符 */
    sockfd_light_state = socket(AF_INET, SOCK_STREAM, 0);
    if (0 > sockfd_light_state) {
        printf("socket 111 error");
    }
    ...
    ret = bind(sockfd_light_state, (struct sockaddr *)&server_addr, sizeof(server_addr));
    if (0 > ret) {
        printf("bind 111 error");
        close(sockfd_light_state);
    }
    /* 使服务器进入监听状态 */
    ret = listen(sockfd_light_state, 50);
    if (0 > ret) {
        printf("listen 111 error");
        close(sockfd_light_state);
    }

    /* 阻塞等待客户端连接 */
    connfd = accept(sockfd_light_state, (struct sockaddr *)&client_addr, (socklen_t *)&addrlen);// 这里需要强制转换一下 addrlen
    if (0 > connfd) {
        printf("accept 111 error");
        close(sockfd_light_state);
    }
    for(int j = 0; j < 6; j ++)
    {
        printf("客户端 3861 成功连接 服务端 3516 111111111 客户端 3861 成功连接 服务端 3516 \n");
        printf("....................................................................................... \n");
        printf("客户端 3861 成功连接 服务端 3516 111111111 客户端 3861 成功连接 服务端 3516 \n");
        printf("....................................................................................... \n");
    }
    ...
    HI_U8 *recvbuf = NULL;
    int recvbuf_max_length = 2;
    if((recvbuf=(unsigned char *)malloc(recvbuf_max_length))!=NULL)// 使用 malloc 分配内存
        memset(recvbuf, 0, recvbuf_max_length);// 使用 memset 初始化
    // printf("wifi_tcp_111_socket_3516_3861 initialize buf \n");

    while(1){
        memset(recvbuf, 0x0, sizeof(recvbuf));// 接收缓冲区清零
        // for(int j = 0; j < 2;j ++)
        // {
        //     printf(" wifi_tcp_111_socket_3516_3861 ********* 3516 recv data send by 3861 ********* \n");
        // }
        ret = recv(connfd, recvbuf, sizeof(recvbuf), 0);// 读数据
        // printf("from app: %s\n", recvbuf);// 将读取到的数据以字符串形式打印出来
        // 如果读取到"1111"则准备向 APP 发送图片
        if (0 == strncmp("11", recvbuf, 2)) {
            red_state = 1;// 表示红灯亮，绿灯灭。
            for(int i = 0; i < 2; i++){
                printf(" ********* 红灯 ********* 红灯*********  红灯 ********* 红灯 ********* \n");
            }
        }
        // 如果读取到"1111"则准备向 APP 发送图片
        if (0 == strncmp("00", recvbuf, 2)) {
            red_state = 0;// 表示绿灯亮，红灯灭。
            for(int i = 0; i < 2; i++){
                printf(" ********* 绿灯 ********* 绿灯*********  绿灯 ********* 绿灯 ********* \n");
            }
        }
...
static void *wifi_tcp_socket_3516_3861_2(void *arg)// 向 3861 发送数据
{
    int ret;
    // for(int i = 0; i < 2; i++){
    //     printf("wifi_tcp_222_socket_3516_3861 循环，为了向 3861 发送指令使警示灯高亮 \n");
    // }
    ...
    sockfd_light_state = socket(AF_INET, SOCK_STREAM, 0);
    if (0 > sockfd_light_state) {
        printf("socket 222 error");
    }

    /* 将套接字与指定端口号进行绑定 */
    server_addr.sin_family = AF_INET;
    server_addr.sin_addr.s_addr = htonl(INADDR_ANY);
    server_addr.sin_port = htons(SERVER_PORT4);

    ret = bind(sockfd_light_state, (struct sockaddr *)&server_addr, sizeof(server_addr));
    if (0 > ret) {
        printf("bind 222 error");
        close(sockfd_light_state);
    }
    /* 使服务器进入监听状态 */
    ret = listen(sockfd_light_state, 50);
    if (0 > ret) {
        printf("listen 222 error");
        close(sockfd_light_state);
    }
    ...
    connfd = accept(sockfd_light_state, (struct sockaddr *)&client_addr, (socklen_t *)&addrlen);// 这里需要强制转换一下 addrlen
    if (0 > connfd) {
        printf("accept 222 error");
        close(sockfd_light_state);
    }
    for(int j = 0; j < 6; j ++)
    {
        printf("客户端 3861 成功连接 服务端 3516 222222222 客户端 3861 成功连接 服务端 3516 \n");
        printf("....................................................................................... \n");
        printf("客户端 3861 成功连接 服务端 3516 222222222 客户端 3861 成功连接 服务端 3516 \n");
        printf("....................................................................................... \n");
    }
    ...
    char warning_light_on[16]="22";
    while(1){
        if(global_trafficlight_violation_2 != 0){// 检测到斑马线上有人就给 3861 发送信息。
            ret = send(connfd, warning_light_on, strlen(warning_light_on), 0);// 发送结束标志，这样可以
            if(0 > ret){printf("send warning_light_on error");}
            for(int i = 0; i < 4; i++){
                printf("给 3861 发送警示灯高亮命令 success 给 3861 发送警示灯高亮命令 success \n");
            }
            global_trafficlight_violation_2 = 0;
        }
        usleep(200000);
    }
}
...
HI_S32 Yolo2HandDetectResnetClassifyLoad(uintptr_t* model)
{
    ...
    HI_CHAR cooperate_audioThreadName[BUFFER_SIZE] = {0};// 音频
    HI_CHAR tcp_app_ThreadName[BUFFER_SIZE] = {0};// tcp 连接手机 APP 
    HI_CHAR wifi_tcp_socket_ThreadName_1[BUFFER_SIZE] = {0};// 3516 和 3861 wifi_tcp_socket 通信 
    HI_CHAR wifi_tcp_socket_ThreadName_2[BUFFER_SIZE] = {0};// 3516 和 3861 wifi_tcp_socket 通信 
...
// ====================================== 连接 Huawei_cloud socket 初始化如下 ======================================
        ...
        if (0 > ret) {
            close(sockfd2);
            sockfd2_available=0;// 表示没有连接上 cloud ，所以后续不会给 cloud 发送图片。
            ret=0;// 防止连接失败对函数返回值造成影响，也就是不管是否连接都可以执行其他程序。
            printf("服务器连接失败 服务器连接失败 服务器连接失败 服务器连接失败 服务器连接失败 服务器连接失败\n");
        }else{
            sockfd2_available=1;// 控制是否给 cloud 发送违规图片的功能
            for(int i = 0; i < 5; i++){
...
// ====================================== 创建音频播放的线程如下 ======================================
        ...
        // ================================================================= 创建配合音频循环播放的计时线程如下
        for(int i = 0; i < 2; i++) {
            printf("创建配合音频循环播放的计时线程开始 创建配合音频循环播放的计时线程开始 \n");
        }
        HI_ASSERT(ret == 0);
        if (snprintf_s(cooperate_audioThreadName, BUFFER_SIZE, BUFFER_SIZE - 1, "cooperate_AudioProcess") < 0) {
            HI_ASSERT(0);
        }
        prctl(PR_SET_NAME, (unsigned long)cooperate_audioThreadName, 0, 0, 0);// 在 prctl.h 中 #define PR_SET_NAME    15

        ret = pthread_create(&g_cooperate_audioProcessThread, NULL, GetCooperateAudioFileName, NULL);// 创建音频播放的线程
        if (ret != 0) {
            SAMPLE_PRT("audio proccess thread creat fail:%s\n", strerror(ret));
            return ret;
        }
        for(int i = 0; i < 2; i++) {
            printf("创建配合音频循环播放的计时线程结束 创建配合音频循环播放的计时线程结束 \n");
        }
        // ================================================================= 创建配合音频循环播放的计时线程如上
...
// ========================== 创建 wifi_tcp_socket_3516_3861_1 的线程如下 ==========================
// 接收 3861 发送的数据
        for(int i = 0; i < 2; i++) {
            printf("创建 wifi_tcp_socket_3516_3861_1 的线程开始 创建 wifi_tcp_socket_3516_3861_1 的线程开始  \n");
        }
        if (snprintf_s(wifi_tcp_socket_ThreadName_1, BUFFER_SIZE, BUFFER_SIZE - 1, "wifi_tcp_socket_3516_3861_1_Process") < 0) {HI_ASSERT(0);}
        ret = pthread_create(&wifi_tcp_socket_ProcessThread_1, NULL, wifi_tcp_socket_3516_3861_1, NULL);// 创建 tcp 连接 app 的线程
        if (ret != 0) {
            SAMPLE_PRT("wifi_tcp_socket_3516_3861_1 proccess thread creat fail:%s\n", strerror(ret));
            return ret;
        }
        for(int i = 0; i < 2; i++) {
            printf("创建 wifi_tcp_socket_3516_3861_1 的线程结束 创建 wifi_tcp_socket_3516_3861_1 的线程结束  \n");
        }
// ========================== 创建 wifi_tcp_socket_3516_3861_1 的线程如上 ==========================



// ========================== 创建 wifi_tcp_socket_3516_3861_2 的线程如下 ==========================
// 向 3861 发送数据
        for(int i = 0; i < 2; i++) {
            printf("创建 wifi_tcp_socket_3516_3861_2 的线程开始 创建 wifi_tcp_socket_3516_3861_2 的线程开始  \n");
        }
        if (snprintf_s(wifi_tcp_socket_ThreadName_2, BUFFER_SIZE, BUFFER_SIZE - 1, "wifi_tcp_socket_3516_3861_2_Process") < 0) {HI_ASSERT(0);}
        ret = pthread_create(&wifi_tcp_socket_ProcessThread_2, NULL, wifi_tcp_socket_3516_3861_2, NULL);// 创建 tcp 连接 app 的线程
        if (ret != 0) {
            SAMPLE_PRT("wifi_tcp_socket_3516_3861_2 proccess thread creat fail:%s\n", strerror(ret));
            return ret;
        }
        for(int i = 0; i < 2; i++) {
            printf("创建 wifi_tcp_socket_3516_3861_2 的线程结束 创建 wifi_tcp_socket_3516_3861_2 的线程结束  \n");
        }
// ========================== 创建 wifi_tcp_socket_3516_3861_2 的线程如上 ==========================

        if (snprintf_s(buf_end, sizeof(buf_end), sizeof(buf_end) - 1, "endendend!@!2!@!") < 0) {
            HI_ASSERT(0);
        }printf("initialize buf_end success \n");
        if (snprintf_s(buf_begin, sizeof(buf_begin), sizeof(buf_begin) - 1, "startstart!@!") < 0) {
            HI_ASSERT(0);
        }printf("initialize buf_begin success \n");
...
// ================================= 后来改为把 srcFrm2 也作为函数的输入参数，然后也是在函数外释放内存。就不用下面的复制了。
    // printf("复制内存 srcFrm 得到 srcFrm2，为了后面画框相关如下 \n");
    ...
    // printf("复制内存 srcFrm 得到 srcFrm2，为了后面画框相关如上 \n");
...
// ====================================== 以下将 yuv 转 jpg 并保存到磁盘 ======================================
        ...
        */
        ...
        if(sockfd1_available==1){// 只有当需要向 app 传输图片时才把摄像头拍摄的图片保存到磁盘，这样可能会缩短一
        // 整个流程的时间。
            if(count_cycle_1 > 10){// 摄像头初始几帧图片可能是坏的，所以不要
        ...
        }// if(sockfd1_available==1)
// ====================================== 以上将 yuv 转 jpg 并保存到磁盘 ======================================

        // usleep(1000);// 这里需要一个延时，因为上面刚刚将一个 yuv 图片数据以 jpg 写入本地，下面就要打开这
        // 个 jpg 图片读取数据。如果不延时，就会出现打开后的文件数据缺失、混乱的问题。
        usleep(50);
...
// =============================================== 输入模型与后处理如下 ===============================================
    ...
    /*
        在 yolov2_hand_detect.c ，78。 
        objs 存储识别到的目标的重要信息：置信度、类别、框的坐标。返回值是识别到的目标数量，存储在 objNum 中。
        ...
        objs[i].cls 存储目标类别，objs[i].box 存储框的坐标。
    */
    printf("模型检测完毕 \n");
    printf(" ************************************** 模型识别到了 %d 个结果 \n", objNum);
    if(red_state == 1){
        global_trafficlight_violation_0 = objNum;// 用作音频播放的标志
        global_trafficlight_violation_1 = objNum;// 用作获取音频类别的标志
        global_trafficlight_violation_2 = objNum;// 用作给 3861 发送信息来高亮警示灯的标志
        printf(" ********* 闯红灯了  闯红灯了 闯红灯了 ********* \n");
        printf(" ********* 闯红灯了  闯红灯了 闯红灯了 ********* \n");
        printf(" ********* 闯红灯了  闯红灯了 闯红灯了 ********* \n");
    }

    int count_class_1 = 0;// 不带头盔的头
    int count_class_2 = 0;// 带头盔的头
    int count_class_3 = 0;// 车轮

    for (int i = 0; i < objNum; i++) {// 有几个目标就循环几次
        cnnBoxs[i] = objs[i].box;
        RectBox *box = &objs[i].box;
        cnnScores[i] = objs[i].score;// 存储识别结果的置信度。置信度大于0.25的就被使用了，但是这太低了。所以
        // 需要对置信度再划分一下，从而实现对音频、给 cloud 发送图片的控制。
        printf(" ************************************** \n");
        printf(" ************************************** \n");
        printf(" ************************************** \n");
        printf("objs[i].cls = %d, {%d, %d, %d, %d} \n", objs[i].cls, objs[i].box.xmin, objs[i].box.xmax, 
            objs[i].box.ymin, objs[i].box.ymax);
        printf(" ************************************** \n");
        printf(" ************************************** \n");
        printf(" ************************************** \n");
        // ==================================== 保存模型输出的框的大小如下 ====================================
        ...
        // printf("保存模型输出的框的大小 \n");
        SAMPLE_PRT("输出模型输出的框的大小， yolo2_out_initial: {%d, %d, %d, %d} \n", box->xmin, box->ymin, box->xmax, box->ymax);
        // ==================================== 保存模型输出的框的大小如上 ====================================
        ...
        // 在 misc_util.c 中，58。按比例转换坐标，转成适配显示屏的框的大小并保存在 box 中。
        SAMPLE_PRT("输出适配显示屏的框的大小， yolo2_out_modify: {%d, %d, %d, %d} \n", box->xmin, box->ymin, box->xmax, box->ymax);
        boxs[i] = *box;
        if(objs[i].cls == 1){
            count_class_1 ++;
        }
        else if(objs[i].cls == 2){
            count_class_2 ++;
        }
        else if(objs[i].cls == 3){
            count_class_3 ++;
        }
    }
    ...
    for(int i = 0; i < 1; i++){
        printf("\t \t 三个类别目标数如下 \n");
        printf("count_class_1 = %d, count_class_2 = %d, count_class_3 = %d \n", 
            count_class_1, count_class_2, count_class_3);
        printf("\t \t 三个类别目标数如上 \n");
    }

    if( (count_class_3 > count_class_2) && (count_class_3 > 0 && count_class_2 >= 0 && count_class_1 > 0) ){// 车轮数大于头盔数说明有人没带头盔
        global_helmet_violation_0 = 1;// 用作音频播放的标志
        global_helmet_violation_1 = 1;// 用作获取音频类别的标志
        global_helmet_violation_2 = 1;// 用作控制给 cloud 发送违规图片的标志
        for(int i = 0; i < 3; i++){
            printf("有人没带头盔 有人没带头盔 有人没带头盔 有人没带头盔 \n");
            printf("count_class_3 = %d, count_class_2 = %d \n", count_class_3, count_class_2);
        }
    }
    count_class_1 = 0;// 不带头盔的头
    count_class_2 = 0;// 带头盔的头
    count_class_3 = 0;// 车轮
    biggestBoxIndex = GetBiggestHandIndex(boxs, objNum);
    SAMPLE_PRT("biggestBoxIndex:%d, objNum:%d \n", biggestBoxIndex, objNum);
    usleep(10);// 可能需要延时，不然后面画线可能冲突，但是好像第一次画线就是会报错。
    /*
     * 当检测到对象时，在 DSTFRM 中绘制一个矩形
     * When an object is detected, a rectangle is drawn in the DSTFRM
     */
    if (biggestBoxIndex >= 0) {// 画框方式是先画最大的框，再画剩下的框
...
// ================================= 从磁盘读取文件或者从内存给 cloud 发送不带框的图片如上 =================================
...
// ===================================== 获取带框的图片并从内存或者从磁盘读取文件再发给 cloud 如下 =====================================
    if(count_cycle == 0){
        count_cycle = 1;
        if(global_helmet_violation_2 > 0){
            if(sockfd2_available==1){
                if(count_cycle_1 > 10){// 摄像头初始几帧图片可能是坏的，所以不要

                    for(int i =0; i < 4; i ++){
                        printf("获取带框的图片并从内存或者从磁盘读取文件再发给 cloud 如下 \n");
                    }
                    ...
                    bgr_to_jpg1(rgb2, jpgFileNamepointer2, image_width, image_height);
                    free(rgb2);
                    usleep(50);// 把图片保存到磁盘 和 打开图片文件并读取数据 之间如果不延时，就可能会出现图片数据缺失、混乱的问题。
        ...
        //         if(count_cycle_1 > 10){// 摄像头初始几帧图片可能是坏的，所以不要
                    ...
                    // send_to_Huawei_cloud2(&jpg_buf2);
                    // free(jpg_buf2.start);
                    printf("获取带框的图片并从内存或者从磁盘读取文件再发给 cloud 如上 \n");
                }
            }
        }
    }
    int cycle_Boundary_point = 10;//
    // int cycle_Boundary_point = 5;// 
    if(global_helmet_violation_2 > 0){// 判断有电动车且不带头盔才计数
        count_cycle++;
        printf(" count_cycle = %d count_cycle = %d count_cycle = %d \n", count_cycle);
        global_helmet_violation_2 = 0;
    }
    if(count_cycle==cycle_Boundary_point) count_cycle = 0;// 当计数到10时重置为0，所以是0,2~9一共9个数。9帧发送1帧。

    count_cycle_1++;// 计数前10帧，摄像头启动初期可能不稳定，所以过滤掉前10帧。
    if(count_cycle_1 > 10) count_cycle_1 = 11;
    // free(srcFrm2);// 把 srcFrm2 作为该函数的输入参数，所以在该函数的上层函数中释放即可。
// ===================================== 获取带框的图片并从内存或者从磁盘读取文件再发给 cloud 如上 =====================================
