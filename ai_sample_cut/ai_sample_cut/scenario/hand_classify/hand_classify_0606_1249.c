...
static int sendto_app1()
{
    ...
    if(send_to_APP==1)
    {
        ...
        else
        {
            ...
            {
                // 发送buffer中的字符串到 client_addr ,就是发送给客户端
                if (sendto(client_socket, buffer, BUFFER_SIZE, 0, (struct sockaddr *)&server_addr, 
            ...
            if (sendto(client_socket, buffer_end, BUFFER_SIZE, 0, (struct sockaddr *)&server_addr, 
                server_addr_length) < 0)
            {
...
// ====================================== udp initial below ======================================  
        ...
        // 创建用于 internet 的流协议(TCP)类型 socket ，用 client_socket 代表客户端 socket  
