...
// 为了得到颜色正确的 jpg ,输入该函数的 图像数据就必须是 bgr 。
    ...
    {
        printf("bgr_to_111_jpg1 can't open %s\n", jpg_file);
        return -1;
    }
...
static int send_to_Huawei_cloud1(HI_CHAR *jpgFileNamepointer0)
{
    ...
    */
    ...
    /*
        ...
        memset(yuvbuf,0,image_width*3);
    */
    int buf_max_length = 120000;// 使用 yuv_bgr_jpg 路线，读取 jpg 的平均字节数为110000。 
    /*
    ...
    */
    ...
    fclose(tocloud_fp);
    free(buf);
    // sleep(1);// 需要延时，否则发送图片会出错。可以在函数外延时。
...
{
    printf("以下读取硬盘 jpg 图片再 udp 发送给 app 流程，开始\n");
    ...
    {
        for(int i=0; i<2; i++){
            printf("send_to_APP = %d send_to_APP = %d ，开启视频传输模式 \n", send_to_APP, send_to_APP);
        }
        ...
        {
            printf(" .................... 打开文件 sendto_APP_file_name0_pointer0 成功 .................... \n");
            for(int i=0; i<3; i++){
                printf(".................................. udp_sendto_APP .................................. \n");
            }
            ...
            if (sendto(udp_client_socket, buffer_end, BUFFER_SIZE, 0, (struct sockaddr *)&server_addr, 
                server_addr_length) < 0)
            {
                printf("Send finish_flag Failed!\n");
            }
            for(int i=0; i<3; i++){
    ...
    for(int i=0; i<1; i++){
            printf("send_to_APP = %d send_to_APP = %d ，不开启视频传输模式 \n", send_to_APP, send_to_APP);
    }
    printf("以上读取硬盘 jpg 图片再 udp 发送给 app ，结束\n", send_to_APP);
}
/* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ sendto_app above ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */ 
...
{// 分别为RGB数据，保存到磁盘的jpg文件名，图片宽，图片长，保存图片的 jpg 格式的内存，图片大小（长度）
    printf("bgr_to_111_jpg 将内存中的 bgr 图片数据转为 jpg 数据并保存到另一块内存中的流程 开始 \n");
    int depth = 3;
    JSAMPROW row_pointer[1];// 指向一行图像数据的指针
    ...
    printf("bgr_to_111_jpg malloc memmory \n");
    unsigned char * partial_jpg_buf=(unsigned char *)malloc(120000);
    ...
    jpeg_set_defaults(&cinfo);
    jpeg_set_quality(&cinfo, JPEG_QUALITY, TRUE );//* limit to baseline-JPEG values
    printf("bgr_to_111_jpg start_compress \n");
    jpeg_start_compress(&cinfo, TRUE);
    int row_stride = width * 3;
    while (cinfo.next_scanline < cinfo.image_height)
    {
    ...
    printf("bgr_to_111_jpg finish compress \n");

    unsigned long length=0;
    length=*partial_jpg_buf_size;
    printf("bgr_to_111_jpg length=%ld, partial_jpg_buf_size=%ld \n", length, *partial_jpg_buf_size);// 110000 是对的
    ...
    */
    ...
    printf("bgr_to_111_jpg malloc memory success \n");
    ...
    // ========================== 以上从内存空间 partial_jpg_buf 拷贝到内存空间 jpg_buf 中 ==========================
        ...
        // FILE *outfile;
        // if((outfile = fopen(jpg_file, "w")) == NULL){
        //     printf("bgr_to_111_jpg can't open %s \n", jpg_file);
        //     return -1;
        // }
        // printf("bgr_to_111_jpg open jpg_file success \n");
        ...
        // {
        //     printf("bgr_to_111_jpg fwrite error fwrite error fwrite error fwrite error fwrite error \n");
        // }
        // printf("bgr_to_111_jpg write over \n");
        // fclose(outfile);
        // printf("bgr_to_111_jpg seve jpg_file success \n");
    ...
    printf("length=%ld, partial_jpg_buf_size=%ld\n", length, *partial_jpg_buf_size);// length=268369 是对的
    printf("bgr_to_111_jpg2 将内存中的 bgr 图片数据转为 jpg 数据并保存到另一块内存中的流程 结束 \n");
    return 0;
}
...
{
    printf("send_to_111_Huawei_cloud2 发送内存中的 jpg 图片流程开始！\n");
    int ret;
    ...
    if(0 > ret){printf("send error send error send error send error send error send error\n");}
    printf("strlen(buf_end)=%d   send end_flag over\n", strlen(buf_end));
    printf("send_to_111_Huawei_cloud2 发送内存中的 jpg 图片流程结束！\n");
...
// ====================================== 以下将 yuv 转 jpg 并保存到磁盘 ======================================
        ...
        */
        ...
        unsigned char *rgb;
        if((rgb=(unsigned char *)malloc(3*image_width*image_height)) != NULL)
            memset(rgb, 0, 3*image_width*image_height);
        printf("yuv_to_jpg 444 \n");
        // void YUV420SP_TO_BGR24(IVE_IMAGE_S *pstImg, unsigned char *rgb, int image_width, int image_height) 
        YUV420SP_TO_BGR24(&img, rgb, image_width, image_height);
        // ============================== convert yuv to bgr above ==============================

        // ============================== convert bgr to jpg below ==============================
        // static int bgr_to_jpg1(unsigned char *pdata, HI_CHAR *jpg_file, int width, int height)
        // bgr_to_jpg1(rgb, jpgFileNamepointer0, image_width, image_height);
        // static int bgr_to_jpg_memmory1(unsigned char *pdata, HI_CHAR *jpg_file, int width, int height, 
            // JPG_BUFTYPE *jpg_buf, unsigned long *partial_jpg_buf_size)
        bgr_to_jpg_memmory1(rgb, jpgFileNamepointer2, image_width, image_height, &jpg_buf2, &jpg_buf_size2);// cloud
        free(rgb);
...
// ====================================== 以上读取硬盘 jpg 图片再发送给 cloud ======================================

        usleep(1000);

// ====================================== 以下读取硬盘 jpg 图片再 udp 发送给 app ======================================
        // static int sendto_app1()
        // sendto_app1();
// ====================================== 以上读取硬盘 jpg 图片再 udp 发送给 app ======================================
