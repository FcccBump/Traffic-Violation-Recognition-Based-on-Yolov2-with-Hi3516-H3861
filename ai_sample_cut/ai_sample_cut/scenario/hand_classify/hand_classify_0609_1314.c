...
{// 分别为RGB数据，保存到磁盘的jpg文件名，图片宽，图片长，保存图片的 jpg 格式的内存，图片大小（长度）
    ...
    unsigned char * partial_jpg_buf=(unsigned char *)malloc(200000);
...
static int sendto_app2(JPG_BUFTYPE *jpg_buf)
{
    ...
    {
        for(int i=0; i<3; i++){
            printf("sendto_111_app2 = %d send_to_APP = %d ，开启视频传输模式 \n", send_to_APP, send_to_APP);
        }
...
// ================================= 复制内存 srcFrm ，为了后面画框相关如下 ================================
    // VIDEO_FRAME_INFO_S *dstFrm2 = NULL;
    // if((dstFrm2=(VIDEO_FRAME_INFO_S *)malloc(sizeof(VIDEO_FRAME_INFO_S))) != NULL)
    //         memset(dstFrm2, 0, sizeof(VIDEO_FRAME_INFO_S));
    // memcpy(dstFrm2, srcFrm, sizeof(VIDEO_FRAME_INFO_S));
...
// ====================================== 以下读取硬盘 jpg 图片或内存再 udp 发送给 app ======================================
        ...
        // send_to_Huawei_cloud2(&jpg_buf1);// 可以发送给 cloud ，说明 jpg_buf1 没问题。
        // jpg_buf1.start = NULL;////////////////////////////////
        free(jpg_buf1.start);////////////////////////////////
        // sleep(3);
...
// 到这里必定错了！！！，没有打印输出 yolo2_out_initial ，如果注释该部分，就有下面的 yolo2_out 。// 其实有打印的，如下所示
/*
连续多次的模型输出的框的结果
...
// ======================================= 从内存给 cloud 发送不带框的图片如上 =======================================

// ======================================== 对复制的内存 dstFrm2 画框如下 ========================================
        // printf("对复制的内存 dstFrm2 画框如下 \n");
        // if (biggestBoxIndex >= 0) {
        //     objBoxs[0] = boxs_initial[biggestBoxIndex];
        //     MppFrmDrawRects(dstFrm2, objBoxs, 1, RGB888_GREEN, DRAW_RETC_THICK);// Target hand objnum is equal to 1
        //     num = 0;
        ...
        //             * Others hand objnum is equal to objnum -1
        //             */
        //             MppFrmDrawRects(dstFrm2, remainingBoxs, objNum - 1, RGB888_RED, DRAW_RETC_THICK);
        //         }
        //     }
        // }
        // printf("对复制的内存 dstFrm2 画框如上 \n");
// ======================================== 对复制的内存 dstFrm2 画框如上 ========================================

// ===================================== 获取带框的图片并从内存发给 cloud 如下 =====================================
    // printf("获取带框的图片并从内存发给 cloud 如下 \n");
    // ret = FrmToOrigImg((VIDEO_FRAME_INFO_S *)dstFrm2, &img2);// 复制数据指针，不复制数据，所以不用释放 img2 。
    // // 但是 dstFrm2 肯定要释放的。
