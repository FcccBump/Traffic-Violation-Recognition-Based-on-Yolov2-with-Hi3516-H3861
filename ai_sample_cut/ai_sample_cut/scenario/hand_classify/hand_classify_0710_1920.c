...
#include "hisignalling_3516_3861_wifi.h"
// 3516 和 3861 wifi 通信如上
...
static int global_helmet_violation = 0;
static int global_trafficlight_violation = 0;
...
// 3516 和 3861 wifi 通信如下
static int wifi_communication_3516_3861_pthread_flag = 0;// 0表示不开启该线程
static pthread_t g_wifi_communicationThread = 0;
...
#define SERVER_PORT3 8088 // 3861 连接 3516 的端口号
static pthread_t wifi_tcp_socket_ProcessThread = 0;
...
static HI_VOID PlayAudio2(const RecogNumInfo items)// 受手势识别模型的输出参数 global_helmet_violation 的控制
{
    ...
    const RecogNumInfo *item = &items;
    uint32_t score = item->score * MULTIPLE_OF_EXPANSION / SCORE_MAX;// 1800*100/4096
    if ((score > AUDIO_SCORE) && (global_helmet_violation != 0 || global_trafficlight_violation != 0) 
        && (global_audio_flag == 0)) {
        // 满足条件才会执行音频播放程序。 AUDIO_SCORE = 40 ，定义在前面 。所以 item->score 最小为1639。
        global_helmet_violation = 0;global_audio_flag = 1;
        g_num = item->num;
        for(int i = 0; i < 2; i++){
            printf("满足条件： global_helmet_violation = %d, global_trafficlight_violation = %d, global_audio_flag = %d, 执行音频播放程序 \n", 
                global_helmet_violation, global_trafficlight_violation, global_audio_flag);
        }
        if (g_num != UNKOWN_WASTE) {// UNKOWN_WASTE = 20
            AudioTest(g_num, -1);// 在 audio_test.c 中，482。 g_num 是识别出来的垃圾类别。
        }
        printf("播放结束时： global_helmet_violation = %d, global_trafficlight_violation = %d, global_audio_flag = %d \n", 
            global_helmet_violation, global_trafficlight_violation, global_audio_flag);
        global_audio_flag = 0;
    }
    g_count = 0;
...
*/
    ...
    // 但是还有下层函数，所以不是一直播放
        ...
        if(global_helmet_violation != 0){
            resBuf.num = 1;// 播放“请佩戴头盔”。优先级更高
        }
        else if(global_trafficlight_violation != 0){
...
int HiSteramingServer(void)
{
    ...
    if (signal(SIGILL, SigIntHandler) == 0) {
        return 0;
    }
...
static HI_VOID* wifi_communication_3516_3861(HI_VOID* arg)// // 3516 和 3861 wifi 通信线程
{
    if(wifi_communication_3516_3861_pthread_flag==1){
        LinkPlatform *link = 0;
        ...
        */
        ...
        while (!g_interrupted) {
            ...
            if (qres != 0) { // Connect with the server
                ...
                if (status == StatusDeviceOffline) {
                    link->discover(link, g_device_type);
                }
                QueryResultFree(qres); // free histreaming agent module
            }
            printf(" 第 %d 次循环发送 \n", led_flag);
            sleep(HISIGNALLING_MSG_HEADER_LEN);// 延时(1)，可能是延时1s。
        }// // 这个循环可能是发送一次数据就会和服务端断开连接，然后再次连接、发送。
        LinkPlatformFree(link);
        LinkAgentFree(linkAgent);
    }// if(wifi_communication_3516_3861_pthread_flag==1){
    return NULL;
...
// ///////////////////////////////////////// 3516 和 3861 wifi_tcp_socket 通信如下
static void *wifi_tcp_socket_3516_3861(void *arg)
{
    int ret;
    for(int i = 0; i < 2; i++){
    ...
    for(int j = 0; j < 10; j ++)
    {
        printf("客户端 3861 开发板成功连接 3516 服务端 客户端 3861 开发板成功连接 3516 服务端 \n");
        printf("....................................................................................... \n");
        printf("客户端 3861 开发板成功连接 3516 服务端 客户端 3861 开发板成功连接 3516 服务端 \n");
        printf("....................................................................................... \n");
    }
    ...
    while(1){
        ...
        usleep(200000);
        ret = send(connfd, LED_buf_on, strlen(LED_buf_on),0);// 发送结束标志，这样可以
        if(0 > ret){printf("send error");}
...
HI_S32 Yolo2HandDetectResnetClassifyLoad(uintptr_t* model)
{
    ...
    HI_CHAR wifi_communication_ThreadName[BUFFER_SIZE] = {0};// 3516 和 3861 wifi 通信 
    HI_CHAR wifi_tcp_socket_ThreadName[BUFFER_SIZE] = {0};// 3516 和 3861 wifi_tcp_socket 通信 
...
// ====================================== 3516 和 3861 wifi 通信 below ====================================== 

        for(int i = 0; i < 2; i++) {
            printf("创建 3516 和 3861 wifi 通信的线程 开始，创建 3516 和 3861 wifi 通信的线程 开始 \n");
        }
        // ret = SkPairCreate(&g_stmChn);// 在 posix_help.c 中，44。配置输入输出通道为0、1
        HI_ASSERT(ret == 0);
        if (snprintf_s(wifi_communication_ThreadName, BUFFER_SIZE, BUFFER_SIZE - 1, "wifi_communication_Process") < 0) {
            HI_ASSERT(0);
        }
        prctl(PR_SET_NAME, (unsigned long)wifi_communication_ThreadName, 0, 0, 0);// 在 prctl.h 中 #define PR_SET_NAME    15
        //  在 prctl.h 中， int prctl (int, ...);// 不懂！！！============================================================

        ret = pthread_create(&g_wifi_communicationThread, NULL, wifi_communication_3516_3861, NULL);// 创建音频播放的线程
        if (ret != 0) {
            SAMPLE_PRT("audio proccess thread creat fail:%s\n", strerror(ret));
            return ret;
        }
        for(int i = 0; i < 2; i++) {
            printf("创建 3516 和 3861 wifi 通信的线程 结束，创建 3516 和 3861 wifi 通信的线程 结束 \n");
        }

// ====================================== 3516 和 3861 wifi 通信 above ====================================== 



// ========================== 创建 wifi_tcp_socket_3516_3861 的线程如下 ==========================
        for(int i = 0; i < 2; i++) {
            printf("创建 wifi_tcp_socket_3516_3861 的线程开始 创建 wifi_tcp_socket_3516_3861 的线程开始  \n");
        }
        if (snprintf_s(wifi_tcp_socket_ThreadName, BUFFER_SIZE, BUFFER_SIZE - 1, "wifi_tcp_socket_3516_3861_Process") < 0) {HI_ASSERT(0);}
        ret = pthread_create(&wifi_tcp_socket_ProcessThread, NULL, wifi_tcp_socket_3516_3861, NULL);// 创建 tcp 连接 app 的线程
        if (ret != 0) {
            SAMPLE_PRT("wifi_tcp_socket_3516_3861 proccess thread creat fail:%s\n", strerror(ret));
            return ret;
        }
        for(int i = 0; i < 2; i++) {
            printf("创建 wifi_tcp_socket_3516_3861 的线程结束 创建 wifi_tcp_socket_3516_3861 的线程结束  \n");
        }
// ========================== 创建 wifi_tcp_socket_3516_3861 的线程如上 ==========================
...
// =============================================== 输入模型与后处理如下 ===============================================
    ...
    printf("模型检测完毕 \n");
    global_helmet_violation = objNum;
    // global_trafficlight_violation = objNum;
    printf("将模型识别结果赋值给全局变量 global_helmet_violation = %d \n", global_helmet_violation);
    // sleep(6);
