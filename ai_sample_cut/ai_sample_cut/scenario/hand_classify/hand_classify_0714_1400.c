...
0714：
    1330：
        启用了从内存直接发送的函数，运行效率并没有提高。运行20分钟后出现错误，又过了一会就崩溃了。
*/
...
{// 分别为RGB数据，保存到磁盘的jpg文件名，图片宽，图片长，保存图片的 jpg 格式的内存，图片大小（长度）
    ...
    // 110000 是对的
// ========================== 以下从内存空间 partial_jpg_buf 拷贝到内存空间 jpg_buf0 中 ==========================
    /*
    ...
    */
    ...
    // // === 使用输入参数 jpg_buf0
    jpg_buf0->length=(int)length;
    printf("bgr_to_111_jpg_memmory1 malloc memory success \n");
    memcpy(jpg_buf0->start, partial_jpg_buf, length);// 使用这个 memcpy ，partial_jpg_buf 必须是 malloc 分配了内存的
    ...
    if(count_cycle==0){
        if(sockfd2_available==1){
            if(count_cycle_1 > 10){// 摄像头初始几帧图片可能是坏的，所以不要
                ...
                int length_jpg_buf2_start = 300000;// 够大
                if((jpg_buf2.start=(uint8_t *)malloc(length_jpg_buf2_start))!=NULL)
                    memset(jpg_buf2.start, 0, length_jpg_buf2_start);
                bgr_to_jpg_memmory1(rgb2, jpgFileNamepointer2, image_width, image_height, &jpg_buf2, &jpg_buf_size2);// cloud
                free(rgb2);
