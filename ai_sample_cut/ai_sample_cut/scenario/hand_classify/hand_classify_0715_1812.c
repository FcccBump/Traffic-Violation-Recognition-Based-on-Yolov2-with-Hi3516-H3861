...
#define SERVER_IP1 "10.139.175.63"// my 热点 0714_1712
...
#define SERVER_IP "10.139.175.63"// my 热点 0714_1712
// udp 相关如上
...
static HI_S32 play_propaganda_slogans = 0;
// 音频相关如上
...
static int count_cycle_2 = 0;// 显示屏显示帧数控制，刷新太快会导致残影
...
static HI_VOID PlayAudio2(const RecogNumInfo items)// 受手势识别模型的输出参数 global_helmet_violation_0 的控制
{
    ...
    if ((score > AUDIO_SCORE) && (global_helmet_violation_0 != 0 || global_trafficlight_violation_0 != 0) 
        ...
        g_num = item->num;
        // for(int i = 0; i < 2; i++){
        //     printf("满足条件： global_helmet_violation_0 = %d, global_trafficlight_violation_0 = %d, global_audio_flag = %d, 执行音频播放程序 \n", 
        //         global_helmet_violation_0, global_trafficlight_violation_0, global_audio_flag);
        // }
        if (g_num != UNKOWN_WASTE) {// UNKOWN_WASTE = 20
            AudioTest(g_num, -1);// 在 audio_test.c 中，482。 g_num 是识别出来的垃圾类别。现在是音频类别别，一共两种。
            // 分别是 1：“请佩戴头盔”，2：“请不要闯红灯”。
        }
        printf("播放结束时： global_helmet_violation_0 = %d, global_trafficlight_violation_0 = %d, global_audio_flag = %d \n", 
            global_helmet_violation_0, global_trafficlight_violation_0, global_audio_flag);
        global_audio_flag = 0;
    }
    else if(play_propaganda_slogans == 1 && global_audio_flag == 0){
        global_audio_flag = 1;
        g_num = item->num;
        if (g_num <= 7 && g_num >= 3) {
...
*/
    ...
    // 但是还有下层函数，所以不是一直播放
        ...
        else if(propaganda_slogans == 3){
            propaganda_slogans = 0;
...
{
    int ret = 0;int i = 0;
    propaganda_slogans = 3;play_propaganda_slogans = 0;
    while(1){
        if(propaganda_slogans > 7){
            propaganda_slogans = 3;
        }
        sleep(50);
        play_propaganda_slogans = 1;
        sleep(10);
        play_propaganda_slogans = 0;
        propaganda_slogans ++;
    }
}
/* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ 音频相关如上 ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */ 
...
// =============================================== 输入模型与后处理如下 ===============================================
    ...
    */
    printf("模型检测完毕 \n");
    printf(" ************************************** 模型识别到了 %d 个结果 \n", objNum);
    // global_helmet_violation_0 = objNum;// 用作音频播放的标志
    // global_helmet_violation_1 = objNum;// 用作获取音频类别的标志
    if(red_state == 1){
    ...
    for (int i = 0; i < objNum; i++) {// 有几个目标就循环几次
        ...
        printf(" ************************************** \n");
        printf(" ************************************** \n");
        printf("objs[i].cls = %d, objs[i].score = %d, {%d, %d, %d, %d} \n", objs[i].cls, objs[i].score, 
            objs[i].box.xmin, objs[i].box.xmax, objs[i].box.ymin, objs[i].box.ymax);
        printf(" ************************************** \n");
        printf(" ************************************** \n");
        printf(" ************************************** \n");
        // ==================================== 保存模型输出的框的大小如下 ====================================
    ...
    if( (count_class_3 > count_class_2) && (count_class_3 > 0 && count_class_2 >= 0) ){// 车轮数大于头盔数说明有人没带头盔
     ...
     */
    // if(count_cycle_2 == 0){// 好像没有用，抽帧了还是残影，可能抽帧不够
    ...
    // }
    count_cycle_2 ++;//
    if(count_cycle_2 > 0) count_cycle_2 = 0;
...
// ===================================== 获取带框的图片并从内存或者从磁盘读取文件再发给 cloud 如下 =====================================
    ...
    if(global_helmet_violation_2 > 0){// 判断有电动车且不带头盔才计数
        count_cycle++;printf(" count_cycle = %d count_cycle = %d count_cycle = %d \n", count_cycle);
