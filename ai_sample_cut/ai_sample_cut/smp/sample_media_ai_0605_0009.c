...
0605:
    0008:
        添加 count_cycle ，用于控制每几帧向 cloud 发送图片。还没测试。
...
static HI_VOID HandClassifyAiProcess2(VIDEO_FRAME_INFO_S frm, VO_LAYER voLayer, VO_CHN voChn, HI_S32 count_cycle)
{// 添加 count_cycle ，用于控制每几帧向 cloud 发送图片。
    int ret;
    if (GetCfgBool("hand_classify_switch:support_hand_classify", true)) {
        if (g_workPlug.model == 0) {
            ret = Yolo2HandDetectResnetClassifyLoad(&g_workPlug.model);// 在 hand_classify.c 中，79。加载模型
        ...
        */
        ...
        ret = Yolo2HandDetectResnetClassifyCal2(g_workPlug.model, &resizeFrm, &frm, count_cycle);
        // 在 hand_classify.c 中，934。将帧缩放后的图片传入模型。
        // 添加 count_cycle ，用于控制每几帧向 cloud 发送图片。
        SAMPLE_CHECK_EXPR_GOTO(ret < 0, HAND_RELEASE,
            "hand classify plug cal FAIL, ret=%#x\n", ret);
...
static HI_VOID* GetVpssChnFrameHandClassify(HI_VOID* arg)
{
    ...
    while (HI_FALSE == g_bAiProcessStopSignal) {
        ...
        HandClassifyAiProcess2(frm, voLayer, voChn, count_cycle);// 添加 count_cycle ，用于控制每几帧向 cloud 发送图片。
        count_cycle++;
